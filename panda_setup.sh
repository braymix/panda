...
# Setup completo - Esegui con: bash panda_setup.sh
#############################################

set -e

echo "🐼 PANDA Setup - Iniziamo!"
echo "================================"

# Creazione struttura cartelle
echo "[1/6] Creazione struttura cartelle..."
mkdir -p ~/panda/{tasks,results,logs,config,scripts}

# Creazione worker principale
echo "[2/6] Creazione worker.py..."
cat << 'WORKER_EOF' > ~/panda/worker.py
#!/usr/bin/env python3
"""
...
def get_pending_tasks():
    """Ritorna lista task pendenti ordinati per priorità"""
    tasks = []
...
def main():
    ...
    while True:
        try:
            ...
            log("🐼 PANDA Worker fermato dall'utente")
            break
        except Exception as e:
...
chmod +x ~/panda/worker.py

# Creazione config di default
echo "[3/6] Creazione configurazione..."
cat << 'CONFIG_EOF' > ~/panda/config/panda.json
{
  "ollama_url": "http://localhost:11434",
  "model": "qwen2.5-coder:3b",
  "check_interval": 10,
...
# Creazione task di esempio
echo "[4/6] Creazione task di esempio..."
cat << 'TASK1_EOF' > ~/panda/tasks/001_hello_world.json
{
  "id": "hello_world",
  "type": "python",
  "priority": 1,
  "prompt": "Scrivi un programma che stampa 'Ciao! PANDA è operativo!' e la data/ora corrente"
}
TASK1_EOF

cat << 'TASK2_EOF' > ~/panda/tasks/002_system_info.json
{
  "id": "system_info",
  "type": "bash",
  "priority": 2,
  "prompt": "Mostra informazioni sul sistema: hostname, uptime, memoria disponibile"
}
TASK2_EOF

cat << 'TASK3_EOF' > ~/panda/tasks/003_create_readme.json
{
  "id": "create_readme",
  "type": "file",
  "priority": 3,
  "filepath": "~/panda/README.md",
  "prompt": "Crea un README per il progetto PANDA (Pana Automating Never Death on Afterwork). Spiega che è un sistema di automazione task con LLM locale che gira h24 su un mini PC Linux. Scrivi in italiano."
}
TASK3_EOF

# Creazione systemd service
echo "[5/6] Creazione systemd service..."
sudo tee /etc/systemd/system/panda.service > /dev/null << SERVICE_EOF
[Unit]
Description=PANDA - Pana Automating Never Death (on) Afterwork
...
ExecStart=/usr/bin/python3 $HOME/panda/worker.py
Restart=always
RestartSec=10

# Logging
StandardOutput=append:$HOME/panda/logs/panda-stdout.log
StandardError=append:$HOME/panda/logs/panda-stderr.log
...
echo "[6/6] Abilitazione servizio systemd..."
sudo systemctl daemon-reload
sudo systemctl enable panda.service

echo ""
echo "================================"
echo "🐼 PANDA Setup completato!"
echo "================================"
echo ""
echo "Struttura creata in ~/panda/"
echo ""
echo "Comandi utili:"
echo "  sudo systemctl start panda    # Avvia PANDA"
echo "  sudo systemctl stop panda     # Ferma PANDA"
echo "  sudo systemctl status panda   # Stato PANDA"
echo "  sudo systemctl restart panda  # Riavvia PANDA"
echo "  tail -f ~/panda/logs/*.log    # Vedi log in tempo reale"
echo ""
echo "Per aggiungere task:"
echo "  Crea file JSON in ~/panda/tasks/"
echo ""
echo "Configurazione:"
echo "  Modifica ~/panda/config/panda.json"
echo "  (hot reload, non serve riavviare)"
echo ""
echo "Task di esempio già pronti in ~/panda/tasks/"
echo ""
echo "Vuoi avviare PANDA ora? Esegui:"
echo "  sudo systemctl start panda"
echo ""
